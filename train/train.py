            ...
            with amp.autocast(enabled=cuda):
                if opt.kd_weights:
                    ...
                    if opt.kd_hard_labels:
                        ...
                        adapted_student = []
                        for adap_layer, sf in zip(compute_loss.feature_adaptation_layers, student_features):
                            adapted_student.append(adap_layer(sf))
                        loss, loss_items = compute_loss(student_pred, targets, adapted_student, teacher_features, teacher_mask, kd_no_imitation=kd_no_imitation)  # loss scaled by batch_size
                else:
