...
def export_tflite(keras_model, im, file, int8, data, ncalib, prefix=colorstr('TensorFlow Lite:'), keep_f32=False):
    ...
    try:
        ...
        if int8:
            ...
            output_path = str(file).replace('.pt', '-int8.tflite')

        tflite_model = converter.convert()
        open(output_path, "wb").write(tflite_model)
        LOGGER.info(f'{prefix} export success, saved as {output_path} ({file_size(output_path):.1f} MB)')
        return output_path
    except Exception as e:
        LOGGER.info(f'\n{prefix} export failure: {e}')
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    if name:
        ...
        output_path = os.path.join(file_path[0], name)
    else:
        output_path = file
